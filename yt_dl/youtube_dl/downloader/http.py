...
class HttpFD(FileDownloader):
    def real_download(self, filename, info_dict):
        ...
        def download():
            ...
            while True:
                ...
                if ctx.stream is None:
                    try:
                        ...
                        print('unable to open for writing: %s' % str(err))
                        return False
                    ...
                    if self.params.get('xattr_set_filesize', False) and data_len is not None:
                        try:
                            ...
                            print(err)
                            print('unable to set filesize xattr: %s' % str(err))

                try:
                    ctx.stream.write(data_block)
                except (IOError, OSError) as err:
                    # self.to_stderr('\n')
                    print('unable to write data: %s' % str(err))
                    return False
            ...
            if ctx.stream is None:
            #     self.to_stderr('\n')
                print('Did not get any data blocks')
                return False
            if ctx.tmpfilename != '-':
                ctx.stream.close()
        ...
        while count <= retries:
            try:
                ...
                count += 1
                if count <= retries:
                    print(e.source_error, count, retries)
                continue
            except NextFragment:
                continue
        ...
        print('giving up after %s retries' % retries)
        return False
